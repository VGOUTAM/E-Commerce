...
//THIS SAVES THE TOKEN INTO DATABASE
userSchema.methods.generateAuthToken = async function(){
  try{
      let tokenCreate= jwt.sign({_id: this._id},process.env.SECRET_KEY);
      this.tokens=this.tokens.concat({ token: tokenCreate })
...
//THIS IS NEEDED TO ACCPET(PARSE) JSON REQUESTS THAT WE RECEIVE FROM FETCH API.
app.use(bodyParser.json());
...
// ADD TO CART PORT ADDS PRODUCT TO DATA BASE
app.post("/addtocart",authenticate, async function(req,res){
  try{
    console.log("Heyy i am in add to cart");
    console.log(req.body);
...
app.get('/logout',(req, res)=> {
  console.log(req.body);
  //LOGING OUT IS NOTHING BUT CLEARING THE COOKIE....
  res.clearCookie('jwtoken',{path: '/'})
